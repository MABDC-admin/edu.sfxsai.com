...
export class FinanceApiService {
  ...
  getFeeTemplates(academicYearId: string): Observable<FeeTemplate[]> {
    return this.http.get<FeeTemplate[]>(`${this.baseUrl}/fee-templates`, this.ayParams(academicYearId)).pipe(
      map((templates) => templates.map((template) => ({
        ...template,
        lineItems: template.lineItems?.length ? template.lineItems : template.feeTemplateLineItems || [],
      }))),
    );
  }
  ...
  downloadLedgerPdf(studentId: string, academicYearId: string): Observable<Blob> {
    return this.http.get(`${this.baseUrl}/ledger/student/${studentId}/pdf`, {
      params: new HttpParams().set('academicYearId', academicYearId),
      responseType: 'blob'
    });
  }

  downloadFinanceDashboardReportPdf(academicYearId: string): Observable<Blob> {
