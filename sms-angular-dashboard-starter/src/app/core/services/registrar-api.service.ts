...
export class RegistrarApiService {
  ...
  approveEnrollment(id: string, payload: { campus?: 'SFXSAI' | 'MABDC' } = {}): Observable<StudentRecord> {
    return this.http.patch<StudentRecord>(`${this.baseUrl}/students/${id}/approve-enrollment`, payload).pipe(
      map((student) => ({
        ...student,
        gradeLevel: normalizeGradeLevel(student.gradeLevel),
      })),
  ...
  moveLearnerGradeSection(id: string, payload: { gradeLevel: string; section: string; movementType: string; effectiveDate: string; reason: string; remarks?: string; requestedBy?: string; academicYearId?: string }): Observable<StudentRecord> {
    return this.http.patch<StudentRecord>(`${this.baseUrl}/students/${id}/move-grade-section`, payload).pipe(
      map((student) => ({
        ...student,
        gradeLevel: normalizeGradeLevel(student.gradeLevel),
      })),
  ...
  disableLearner(id: string, payload: { movementType: 'Dropout' | 'Transfer Out'; effectiveDate: string; reason: string; remarks?: string; requestedBy?: string }): Observable<StudentRecord> {
    return this.http.patch<StudentRecord>(`${this.baseUrl}/students/${id}/disable`, payload).pipe(
      map((student) => ({
        ...student,
        gradeLevel: normalizeGradeLevel(student.gradeLevel),
      })),
  ...
  dropoutStudent(id: string, payload: { effectiveDate: string; reason: string; remarks?: string; requestedBy?: string }): Observable<StudentRecord> {
    return this.disableLearner(id, { ...payload, movementType: 'Dropout' });
  }
