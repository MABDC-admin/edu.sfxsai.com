...
const STORAGE_KEY = 'sfxsai.student.portal.state.v2';
const today = new Date().toISOString().slice(0, 10);
const gradeSevenSubjects = getSubjectsForGradeLevel('Grade 7');
const classIdForSubject = (subject: string): string => `class-${subject
  .toLowerCase()
  .replace(/\([^)]*\)/g, '')
  .replace(/[^a-z0-9]+/g, '-')
  .replace(/^-|-$/g, '')}`;
const teacherNames = ['Ms. Maria Santos', 'Mr. Carlo Reyes', 'Ms. Elena Cruz', 'Mr. Adrian Lim'];
const scheduleSlots = ['Mon/Wed 8:00 AM', 'Mon/Wed 9:00 AM', 'Tue/Thu 8:00 AM', 'Tue/Thu 9:00 AM', 'Fri 8:00 AM', 'Fri 9:00 AM'];
...
const initialState: StudentPortalState = {
  ...
  classes: gradeSevenSubjects.map((subject, index) => ({
    id: classIdForSubject(subject),
    subject,
    teacher: teacherNames[index % teacherNames.length],
    schedule: scheduleSlots[index % scheduleSlots.length],
    room: `Room ${201 + index}`,
    progress: 58 + ((index * 7) % 34)
  })),
  lessons: [
    { id: 'lesson-1', classId: 'class-mathematics', title: 'Equivalent Fractions', description: 'Compare, simplify, and order fractions.', status: 'Viewed' },
  ...
  assignments: [
    { id: 'assignment-1', classId: 'class-mathematics', title: 'Fraction Worksheet', dueDate: '2026-06-15', status: 'In progress' },
  ...
  attendance: [
    { id: 'att-1', classId: 'class-mathematics', date: today, status: 'Present' },
  ...
  grades: [
    { id: 'grade-1', classId: 'class-mathematics', quarter: 'Q1', written: 91, performance: 93, exam: 92 },
  ...
  resources: [
    { id: 'resource-1', classId: 'class-mathematics', title: 'Fractions Reviewer', subject: 'Mathematics', type: 'PDF', size: '2.1 MB' },
  ...
  announcements: [
    ...
    { id: 'ann-2', scope: 'Class', classId: 'class-mathematics', title: 'Math Quiz', body: 'Review equivalent fractions and mixed numbers.', postedAt: '2026-06-12', important: false, read: false },
  ],
  messages: [
