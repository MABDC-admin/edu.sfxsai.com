...
export class StudentPortalComponent implements OnInit {
  ...
  readonly selectedClassId = signal('class-mathematics');
  ...
  readonly learnerInitials = computed(() =>
  ...
  readonly nextAssignment = computed(() => this.pendingAssignments()[0] ?? this.state().assignments[0]);
  readonly firstUnreadAnnouncement = computed(() => this.state().announcements.find(item => !item.read) ?? this.state().announcements[0]);
  ...
  subjectIcon(subject: string): string {
    ...
    if (normalized.includes('english') || normalized.includes('filipino') || normalized.includes('language')) return 'menu_book';
    if (normalized.includes('mapeh') || normalized.includes('music') || normalized.includes('arts')) return 'palette';
    if (normalized.includes('tle') || normalized.includes('technology')) return 'construction';
    if (normalized.includes('panlipunan') || normalized.includes('social')) return 'public';
    if (normalized.includes('pagpapakatao') || normalized.includes('values')) return 'favorite';
    return 'school';
  }
  ...
  eventTone(eventType: string | null | undefined): string {
    const normalized = String(eventType || '').toLowerCase();
