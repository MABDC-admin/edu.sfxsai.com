...
export class LearnerProfileComponent implements OnInit {
  ...
  get selectedMoveSections() {
    const targetGrade = this.formData.gradeLevel || this.student?.gradeLevel;
    return this.sections.filter(s => gradeLevelMatches(s.gradeLevel, targetGrade));
  }
  ...
  openModal(type: 'sibling' | 'academic' | 'behavior' | 'fee' | 'edit-info' | 'move-section') {
    ...
    if (type === 'edit-info') {
      this.formData = { 
        ...
        motherTongue: this.student.motherTongue,
        dialect: this.student.dialect,
    ...
    if (type === 'move-section') this.formData = {
  ...
  saveModal() {
    ...
    if (this.modalType === 'sibling') {
      ...
      const targetSection = this.sections.find(s => s.id === this.formData.newSection);
      const reason = String(this.formData.reason || '').trim();
      if (!this.formData.gradeLevel || !targetSection || !reason) {
        this.showToast('Move incomplete', 'Select the target grade, section, and enter the movement reason.', 'warning');
        return;
      }

      this.api.moveLearnerGradeSection(id, {
        gradeLevel: this.formData.gradeLevel,
        section: targetSection.sectionName,
        movementType: this.formData.movementType || 'Correction',
        effectiveDate: new Date(this.formData.effectiveDate || Date.now()).toISOString(),
        reason,
        remarks: String(this.formData.remarks || '').trim() || undefined,
        requestedBy: 'Registrar',
        academicYearId: targetSection.academicYearId || this.api.getActiveAcademicYearId()
      }).subscribe({
        next: (res) => {
          this.student = { ...this.student, ...res };
          this.closeModal();
          this.showToast('Learner moved', `Moved to ${displayGradeLevel(res.gradeLevel)} - ${res.section}.`, 'success');
        },
        error: () => this.showToast('Move failed', 'Unable to move the learner. Please check the target grade and section.', 'error')
      });
    }
  }
