...
const droppedOutStatuses = new Set(['Dropped Out']);
...
export function buildRegistrarDashboardMetrics(
  ...
  // Grade Level Stats
  const dynamicGradeLevels = Array.from(new Set(enrolledStudents.map(s => s.gradeLevel))).filter(Boolean).sort();
  const colors = [
  ...
  const gradeLevelStats = dynamicGradeLevels.map((grade, index) => {
    const count = enrolledStudents.filter(s => s.gradeLevel === grade).length;
  ...
  return {
    ...
    droppedOut: students.filter(student => droppedOutStatuses.has(student.enrollmentStatus)).length,
