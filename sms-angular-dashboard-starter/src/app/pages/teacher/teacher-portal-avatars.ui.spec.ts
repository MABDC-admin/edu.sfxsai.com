...
assert.ok(
  avatarHolderCount >= 2,
  `Expected learner avatar holders in attendance and profile surfaces; found ${avatarHolderCount}.`,
);
assert.match(template, /class="[^"]*\bclasses-card-avatar\b[^"]*"/);
assert.match(template, /\*ngIf="learnerAvatarSource\(student\); else (learnerInitials|classesLearnerInitials)"/);
