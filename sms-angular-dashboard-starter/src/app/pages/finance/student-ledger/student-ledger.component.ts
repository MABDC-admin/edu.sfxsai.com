...
export class StudentLedgerComponent implements OnInit {
  ...
  get ledgerFrozen() {
    return isFrozenFinanceAssessment(this.ledger);
  }
  ...
  downloadBackendPdf() {
    ...
    this.error = '';
    this.finance.downloadLedgerPdf(this.selectedStudentId, this.academicYear.id).subscribe({
      next: (blob) => {
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'Student_Ledger_' + this.selectedStudentId + '.pdf';
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        this.message = 'PDF downloaded successfully.';
      },
      error: (err) => {
        this.message = '';
