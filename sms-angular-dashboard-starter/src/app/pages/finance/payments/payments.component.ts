...
import { isFrozenFinanceAssessment } from '../../../core/services/finance-learner-status.util';
...
export class PaymentsComponent implements OnInit {
  ...
  get payableAssessments() {
    return this.assessments.filter((assessment) => !isFrozenFinanceAssessment(assessment));
  }

  get selectedAssessment() {
    return this.payableAssessments.find((assessment) => assessment.id === this.selectedAssessmentId);
  }
  ...
  loadAssessments() {
    forkJoin({
      ...
      next: ({ assessments, payments }) => {
        ...
        if (this.selectedAssessmentId && !this.payableAssessments.some((assessment) => assessment.id === this.selectedAssessmentId)) {
  ...
  recordPayment() {
    ...
    if (isFrozenFinanceAssessment(assessment)) {
      this.error = 'This learner ledger is frozen because the learner is deactivated.';
      return;
    }
  ...
  deletePayment(payment: any) {
    if (confirm(`Are you sure you want to delete receipt ${payment.receiptNumber}?`)) {
      (this.finance as any).deletePayment?.(payment.id).subscribe({
