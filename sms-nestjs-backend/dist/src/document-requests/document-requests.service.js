...
let DocumentRequestsService = class DocumentRequestsService {
    ...
    async findAll(ayId) {
        return this.drizzle.db.query.documentRequest.findMany({
            where: ayId ? (0, drizzle_orm_1.eq)(schema.documentRequest.academicYearId, ayId) : undefined,
        });
    }
    async findOne(id) {
        return this.drizzle.db.query.documentRequest.findFirst({
            where: (0, drizzle_orm_1.eq)(schema.documentRequest.id, id),
        });
    }
    ...
    async remove(id) {
        const [deleted] = await this.drizzle.db.delete(schema.documentRequest).where((0, drizzle_orm_1.eq)(schema.documentRequest.id, id)).returning();
        return deleted;
    }
};
