...
let AcademicRecordsService = class AcademicRecordsService {
    ...
    async findAll(ayId) {
        return this.drizzle.db.query.academicRecord.findMany({
            where: ayId ? (0, drizzle_orm_1.eq)(schema.academicRecord.academicYearId, ayId) : undefined,
        });
    }
    async findOne(id) {
        return this.drizzle.db.query.academicRecord.findFirst({
            where: (0, drizzle_orm_1.eq)(schema.academicRecord.id, id),
        });
    }
    ...
    async remove(id) {
        const [deleted] = await this.drizzle.db.delete(schema.academicRecord).where((0, drizzle_orm_1.eq)(schema.academicRecord.id, id)).returning();
        return deleted;
    }
};
