...
let AcademicYearsService = class AcademicYearsService {
    ...
    async findOne(id) {
        return this.drizzle.db.query.academicYear.findFirst({ where: (0, drizzle_orm_1.eq)(schema.academicYear.id, id) });
    }
    async update(id, data) {
        const [updated] = await this.drizzle.db
            .update(schema.academicYear)
            .set(data)
            .where((0, drizzle_orm_1.eq)(schema.academicYear.id, id))
            .returning();
        return updated;
    }
    async remove(id) {
        const [deleted] = await this.drizzle.db.delete(schema.academicYear).where((0, drizzle_orm_1.eq)(schema.academicYear.id, id)).returning();
        return deleted;
    }
};
