...
describe('registrar controller role metadata', () => {
  ...
  it('allows teacher read access to section metadata endpoints', () => {
    expect(reflector.get(ROLES_KEY, SectionsController)).toEqual([
      'REGISTRAR',
      'TEACHER',
    ]);
    expect(reflector.get(ROLES_KEY, SectionsController.prototype.findAll)).toEqual([
      'REGISTRAR',
      'TEACHER',
    ]);
    expect(reflector.get(ROLES_KEY, SectionsController.prototype.findOne)).toEqual([
      'REGISTRAR',
      'TEACHER',
    ]);
  });

  it('allows principal, finance, and teacher read-only access to student lookup endpoints', () => {
    expect(
      reflector.get(ROLES_KEY, StudentsController.prototype.findAll),
    ).toEqual(['REGISTRAR', 'PRINCIPAL', 'FINANCE', 'TEACHER']);
    expect(
